...
def voucher_sc(en,window1):
    ...
    def lb_bind(event):
        ...
        root.destroy()
    listbox = Listbox(root,width=62,height=40,selectbackground="steelblue",background="lightsteelblue",font=('times',13)) 
...
def inv_n(en,file1,tit,window1):
    ...
    def lb_bind(event):
        en.insert(END,select[-1])
        root.destroy()
    listbox = Listbox(root,width=62,height=40,selectbackground="brown",background="lavender",font=('times',13)) 
    listbox.bind('<<ListboxSelect>>',CurSelet)
    root.bind("<Return>",lb_bind)
    listbox.pack(side = LEFT, fill = BOTH) 
    scrollbar = Scrollbar(root) 
